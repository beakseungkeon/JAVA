...
public class ListEx1 {
	...
	public static void main(String[] args) {
		// 제네릭 인터페이스인 List의 타입을 저장하지 않은 경우
		//타입에 최상위 클래스인 Object가 자동으로 들어감
		//get()을 이용할 때 클래스 형변환을 이용해야함
		...
		/* List3 는 List인터페이스를구현한 구현 클래스의 객체중 아무나 올 수 있다,
		 * list3 = new ArrayList<String();
		 * list3 = new LinkedList<String();
		 * list4 는 ArrayList 클래스를 이용한 객체만 올 수 있다.
		 ...
		 * remove(객체)Collection,객체와 일치하는 요소를 젝 후 제거 여부를 반환
		 */
		list5.remove(0);
		list5.remove("def");
		...
		/*
		 * sort():List,람다식을 이용해서 정렬기준을 정해주면 정해진 정렬 방법  기준대로 리스트를 정렬
		 * 정렬기준 정수로 알려주고 음수이면 앞,0이면 같은 위치, 양수이면 뒤 
		 * compator 인터페이스를 구현한 클래스의 객체가 필요
		 * 일반적으로는 람다식을 이용하여 익명 객체를 생성해서 황용
		 */
		list5.sort((s1,s2)->s1.compareTo(s2));
		/*
		 * Collections.sort():리스트의 타입이 comparrable 인터페이스를 구현한 구현 클래스인 경우
		 * 해당 기준을 이용하여 정렬
		 */
